...
import com.smile67.prize.commons.db.service.CardGameProductService;
import com.smile67.prize.commons.db.service.CardGameRulesService;
...
public class GameTask {
    ...
    public void execute() {
        ...
        list.forEach(game -> {
            ...
            log.info("game start time:{},game end time:{} 分钟 or 秒钟?", start, end);
...
//            long expire = -1; //永不过期
            ...
            log.info("load product type size:{}", productMap.size() + "/n");
            log.info("load product type:{}", productMap);

            // 奖品数量等配置信息
            Map queryMap = new HashMap();
            queryMap.put("gameid", game.getId());
            List<CardGameProduct> gameProducts = gameProductService.listByMap(queryMap);
            log.info("load bind product size:{}", gameProducts.size() + "/n");
            log.info("load bind product:{}", gameProducts);
            ...
            gameProducts.forEach(cgp -> {
                ...
                for (int i = 0; i < cgp.getAmount(); i++) {
                    ...
                    //为什么乘1000，再额外加一个随机数呢？ - 防止：时间段短奖品多时重复导致令牌重复
